...
async def selective_access_log(request, call_next):
    tokens = None
    user = None
    scoped_request = request.url.path.startswith(_USER_SCOPED_HTTP_PREFIXES)
    try:
        ...
        if user is None and scoped_request:
            response = JSONResponse({"detail": "Not authenticated"}, status_code=401)
        else:
            if user is not None:
                tokens = set_current_user(user[0], user[1])
            try:
                response = await call_next(request)
            finally:
