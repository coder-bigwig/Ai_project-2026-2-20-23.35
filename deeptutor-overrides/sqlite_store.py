...
class SQLiteSessionStore:
    ...
    def _initialize(self) -> None:
        with _configure_connection(
            sqlite3.connect(self.db_path, timeout=SQLITE_BUSY_TIMEOUT_MS / 1000)
        ) as conn:
            conn.executescript(
                """
...
_store_instances: dict[str, SQLiteSessionStore] = {}


class ContextSQLiteSessionStore:
    """Dispatch SQLite store calls to the current request/WS user's database."""

    def _store_for_current_user(self) -> SQLiteSessionStore:
        from deeptutor.user_context import get_current_user_id, chat_db_path

        user_id = get_current_user_id()
        if not user_id:
            raise RuntimeError("DeepTutor user context is not set")
        store = _store_instances.get(user_id)
        if store is None:
            store = SQLiteSessionStore(db_path=chat_db_path(user_id))
            _store_instances[user_id] = store
        return store

    def __getattr__(self, name: str):
        return getattr(self._store_for_current_user(), name)


_context_store = ContextSQLiteSessionStore()


def get_sqlite_session_store() -> ContextSQLiteSessionStore:
