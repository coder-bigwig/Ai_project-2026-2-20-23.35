...
async def require_auth(
    ...
    NOTE (override): Made async so that ``set_current_user`` (a ContextVar)
    runs in the request's async task. FastAPI executes sync deps in a thread
    pool, which has its OWN ContextVar snapshot — mutations there do NOT
    propagate to the endpoint handler, causing per-user PathService to fall
    back to legacy shared paths.
    """
    if not AUTH_ENABLED:
