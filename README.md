...
| 目标                         | 使用方式                                                    | 入口                    |
| ---------------------------- | ----------------------------------------------------------- | ----------------------- |
| 本地完整运行（推荐先跑这个） | `start.bat` (Windows) / `./start.sh` (macOS·Linux) / `docker-compose.yml` | `http://localhost:8080` |
| 前端热更新开发               | `start-dev.bat` (Windows) / `./start-dev.sh` (macOS·Linux)  | `http://localhost:3000` |
...
├── start.bat / start.sh        # 一键启动（完整模式，Windows / macOS·Linux）
├── start-dev.bat / start-dev.sh  # 开发模式（前端热更新）
└── stop-dev.bat / stop-dev.sh  # 停止开发模式后端容器
```
...
> 首次执行 `.sh` 前如果提示权限不足，跑一次 `chmod +x start.sh start-dev.sh stop-dev.sh`。
