...
async def init_db_schema() -> None:
    ...
    async with _engine.begin() as conn:
        ...
        resource_folders_table = (
            f'"{POSTGRES_SCHEMA}"."resource_folders"'
            if POSTGRES_SCHEMA and POSTGRES_SCHEMA != "public"
            else "resource_folders"
        )
        await conn.execute(text(f"ALTER TABLE {resource_folders_table} ADD COLUMN IF NOT EXISTS course_id VARCHAR(64)"))
        await conn.execute(text(f"ALTER TABLE {resource_folders_table} DROP CONSTRAINT IF EXISTS uq_resource_folders_owner_name"))
        await conn.execute(
            text(
                f'CREATE UNIQUE INDEX IF NOT EXISTS "ux_resource_folders_owner_course_name_expr" '
                f"ON {resource_folders_table} (owner_username, COALESCE(course_id, ''), lower(name))"
            )
        )
        await conn.execute(
            text(
                f'CREATE INDEX IF NOT EXISTS "ix_resource_folders_course_updated_at" '
                f"ON {resource_folders_table} (course_id, updated_at)"
            )
        )
