...
from .kv_policy_service import get_kv_json
...
def _user_kv_key(username: str) -> str:
    return f"{USAGE_MONITOR_USER_KEY_PREFIX}{normalize_text(username)}"
...
async def load_usage_monitor_state(db: AsyncSession) -> dict:
    legacy_raw = await get_kv_json(db, USAGE_MONITOR_KV_KEY, _default_state())
    users_raw = legacy_raw.get("users", {}) if isinstance(legacy_raw, dict) else {}
    users: dict[str, dict] = {}
    ...
    for row in await repo.list_by_prefix(USAGE_MONITOR_USER_KEY_PREFIX):
        normalized_username = normalize_text(row.key[len(USAGE_MONITOR_USER_KEY_PREFIX) :])
        if not normalized_username:
            continue
        users[normalized_username] = _normalize_user_entry(row.value_json)

    return {
        "version": USAGE_MONITOR_VERSION,
        "updated_at": normalize_text((legacy_raw or {}).get("updated_at")),
        "users": users,
    }
...
async def save_usage_monitor_state(db: AsyncSession, state: dict) -> None:
    ...
    for username, entry in serialized_users.items():
        await save_usage_monitor_user(db, username=username, entry=entry)


async def load_usage_monitor_user(db: AsyncSession, *, username: str, role: str = "student") -> dict:
    normalized_username = normalize_text(username)
    if not normalized_username:
        return _normalize_user_entry(None, role=role)

    repo = KVStoreRepository(db)
    row = await repo.get(_user_kv_key(normalized_username))
    if row is not None and isinstance(row.value_json, dict):
        return _normalize_user_entry(row.value_json, role=role)

    legacy_raw = await get_kv_json(db, USAGE_MONITOR_KV_KEY, _default_state())
    legacy_users = legacy_raw.get("users", {}) if isinstance(legacy_raw, dict) else {}
    legacy_entry = legacy_users.get(normalized_username) if isinstance(legacy_users, dict) else None
    return _normalize_user_entry(legacy_entry, role=role)


async def save_usage_monitor_user(db: AsyncSession, *, username: str, entry: dict) -> None:
    normalized_username = normalize_text(username)
    if not normalized_username:
        return
    payload = {
        **_normalize_user_entry(entry),
        "version": USAGE_MONITOR_VERSION,
...
async def sync_and_build_jupyter_usage_report(
    ...
    *,
    ...
    if changed:
        users_state = state.get("users", {}) if isinstance(state, dict) else {}
        for username in sorted(changed_users):
