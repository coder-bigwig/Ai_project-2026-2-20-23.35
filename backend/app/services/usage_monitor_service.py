...
USAGE_MONITOR_USER_KEY_PREFIX = f"{USAGE_MONITOR_KV_KEY}:"
...
def _usage_monitor_user_key(username: str) -> str:
    return f"{USAGE_MONITOR_USER_KEY_PREFIX}{username}"


def _username_from_usage_monitor_key(key: str) -> str:
    raw = normalize_text(key)
    if not raw.startswith(USAGE_MONITOR_USER_KEY_PREFIX):
        return ""
    return normalize_text(raw[len(USAGE_MONITOR_USER_KEY_PREFIX) :])
...
def _normalize_persisted_user_payload(username: str, payload: dict | None) -> tuple[str, dict] | None:
    source = payload if isinstance(payload, dict) else {}
    normalized_username = normalize_text(source.get("username")) or normalize_text(username)
    if not normalized_username:
        return None
    entry_source = source.get("entry") if isinstance(source.get("entry"), dict) else source
    return normalized_username, _normalize_user_entry(entry_source)


def _serialize_user_payload(username: str, entry: dict) -> dict:
    normalized_username = normalize_text(username)
    normalized_entry = _normalize_user_entry(entry)
    normalized_entry["updated_at"] = _to_iso(_now_utc())
    return {
        "version": USAGE_MONITOR_VERSION,
...
async def load_usage_monitor_state(db: AsyncSession) -> dict:
    ...
    for row in await KVStoreRepository(db).list_by_prefix(USAGE_MONITOR_USER_KEY_PREFIX):
        resolved = _normalize_persisted_user_payload(
            _username_from_usage_monitor_key(row.key),
            row.value_json if isinstance(row.value_json, dict) else {},
        )
        if resolved is None:
            continue
        username, entry = resolved
...
async def save_usage_monitor_state(
    db: AsyncSession,
    state: dict,
    ...
    target_usernames = {
        ...
        for username in (usernames if usernames is not None else users_raw.keys())
        if normalize_text(username)
    }
    for username in sorted(target_usernames):
        item = users_raw.get(username)
        if item is None:
            continue
        await upsert_kv_json(
            db,
            _usage_monitor_user_key(username),
            _serialize_user_payload(username, item),
        )
...
async def record_jupyter_session_start(
    ...
    *,
    ...
    if changed:
        await save_usage_monitor_state(db, state, usernames={normalized_username})
    return changed
...
async def sync_and_build_jupyter_usage_report(
    ...
    *,
    ...
    for username in sorted(usernames):
        ...
        if user_changed:
            changed_usernames.add(username)
    ...
    if changed_usernames:
        await save_usage_monitor_state(db, state, usernames=changed_usernames)

    return _build_usage_report(live_rows, now=now, timeout_seconds=timeout_seconds), bool(changed_usernames)
