...
class TeacherService:
    ...
    async def _ensure_course_resource_folder(self, course_id: str, folder_id: str):
        normalized_course_id = normalize_text(course_id)
    ...
    async def get_teacher_publish_targets(self, teacher_username: str):
        ...
        class_owner_map = {
            normalize_text(row.name): normalize_text(row.created_by)
            for row in class_rows
            if normalize_text(row.name)
        }

        def shared_teachers_for(row) -> set[str]:
            ...
            if not isinstance(raw_shared, list):
                return set()
            return {normalize_text(item) for item in raw_shared if normalize_text(item)}
        ...
        for row in class_rows:
            class_name = normalize_text(row.name)
            if class_name and (role == "admin" or normalize_text(row.created_by) == normalized_teacher):
                visible_class_names.add(class_name)

        classes = []
        seen_class_names = set()
        for row in sorted(class_rows, key=lambda item: normalize_text(item.name)):
            class_name = normalize_text(row.name)
            if not class_name or class_name not in visible_class_names or class_name in seen_class_names:
                continue
    ...
    async def list_course_resource_folders(self, course_id: str, teacher_username: str):
        course, _, _ = await self._ensure_owned_course(course_id, teacher_username)
        normalized_course_id = normalize_text(course.id)
        folders = await ResourceFolderRepository(self.db).list_by_course(normalized_course_id)
        resource_repo = ResourceRepository(self.db)
        payload_items = [
            self._folder_payload(folder, await resource_repo.count_by_folder(folder.id))
            for folder in folders
        ]
        payload_items.sort(key=lambda item: item.get("created_at") or datetime.min, reverse=True)
        return {"total": len(payload_items), "items": payload_items}

    async def create_course_resource_folder(self, course_id: str, teacher_username: str, name: str):
        course, normalized_teacher, _ = await self._ensure_owned_course(course_id, teacher_username)
        normalized_course_id = normalize_text(course.id)
        folder_name = self._sanitize_resource_folder_name(name)
        folder_repo = ResourceFolderRepository(self.db)
        if await folder_repo.find_by_owner_and_name(course.created_by, folder_name, course_id=normalized_course_id):
        ...
        folder = await folder_repo.create(
            {
                "id": str(uuid.uuid4()),
                "name": folder_name,
                "owner_username": course.created_by,
                "created_by": normalized_teacher,
        ...
        await append_operation_log(
            self.db,
            operator=normalized_teacher,
            action="course_resource_folders.create",
            target=normalized_course_id,
            detail=f"folder_id={folder.id}, name={folder_name}",
        )
        await self._commit()
        return self._folder_payload(folder, 0)

    async def update_course_resource_folder(self, course_id: str, folder_id: str, teacher_username: str, name: str):
        course, normalized_teacher, _ = await self._ensure_owned_course(course_id, teacher_username)
        normalized_course_id = normalize_text(course.id)
        folder = await self._ensure_course_resource_folder(normalized_course_id, folder_id)
        folder_name = self._sanitize_resource_folder_name(name)
        existing = await ResourceFolderRepository(self.db).find_by_owner_and_name(
            course.created_by,
            folder_name,
            course_id=normalized_course_id,
        )
        ...
        course.updated_at = folder.updated_at
        await append_operation_log(
            self.db,
            operator=normalized_teacher,
            action="course_resource_folders.rename",
            target=normalized_course_id,
            detail=f"folder_id={folder.id}, name={folder_name}",
        )
        await self._commit()
    ...
    async def delete_course_resource_folder(self, course_id: str, folder_id: str, teacher_username: str):
        course, normalized_teacher, _ = await self._ensure_owned_course(course_id, teacher_username)
        normalized_course_id = normalize_text(course.id)
        folder = await self._ensure_course_resource_folder(normalized_course_id, folder_id)
        count = await ResourceRepository(self.db).count_by_folder(folder.id)
        if count > 0:
            raise HTTPException(status_code=409, detail="文件夹内还有课程资料，不能删除")
        await ResourceFolderRepository(self.db).delete(folder.id)
        course.updated_at = datetime.now()
        await append_operation_log(
            self.db,
            operator=normalized_teacher,
            action="course_resource_folders.delete",
            target=normalized_course_id,
            detail=f"folder_id={folder.id}, name={folder.name}",
        )
        await self._commit()
        return {"message": "课程文件夹已删除", "id": folder_id}
    ...
    async def upload_course_resource_file(
        ...
        if normalized_folder_id:
            folder = await self._ensure_course_resource_folder(normalized_course_id, normalized_folder_id)
        ...
        if folder is not None:
            course_upload_dir = os.path.join(UPLOAD_DIR, "course_resources", normalized_course_id, "folders", folder.id)
        else:
            course_upload_dir = os.path.join(UPLOAD_DIR, "course_resources", normalized_course_id, "root")
        ...
        await append_operation_log(
            ...
            target=normalized_course_id,
            detail=f"resource_id={resource_id}, folder_id={getattr(folder, 'id', '')}, filename={original_filename}",
        )
        await self._commit()
    ...
    async def list_course_resource_files(
        ...
        normalized_folder_id = normalize_text(folder_id)
        if normalized_folder_id:
            await self._ensure_course_resource_folder(normalized_course_id, normalized_folder_id)
    ...
    async def delete_course_resource_file(self, course_id: str, resource_id: str, teacher_username: str):
        ...
        if normalize_text(getattr(row, "folder_id", "")):
            folder = await self._ensure_course_resource_folder(normalized_course_id, row.folder_id)
