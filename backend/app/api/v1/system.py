...
@router.get("/")
def root():
    return {"message": "教学创新实践平台 API", "version": "1.0.0"}
...
async def metrics(db: AsyncSession = Depends(get_db)):
    ...
    ai_invocation_log_count = await AIInvocationLogRepository(db).count()
    ...
    lines = [
        ...
        "# HELP training_backend_ai_invocation_logs_total Total AI invocation logs stored in PostgreSQL.",
        "# TYPE training_backend_ai_invocation_logs_total gauge",
        f"training_backend_ai_invocation_logs_total {ai_invocation_log_count}",
    ]
    return "\n".join(lines) + "\n"
