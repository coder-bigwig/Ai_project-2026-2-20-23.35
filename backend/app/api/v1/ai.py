...
from ...config import AI_INVOCATION_LOG_SECRET
from ...db.session import get_db
from ...repositories import KVStoreRepository
from ...services.ai_invocation_log_service import append_ai_invocation_log
...
async def _safe_append_ai_invocation_log(db: AsyncSession, payload: dict) -> None:
    try:
        await append_ai_invocation_log(db, payload)
        await db.commit()
    except Exception:
        await db.rollback()


async def ingest_ai_invocation_log(
    payload: Dict,
    x_ai_log_secret: Optional[str] = Header(default=None, alias="X-AI-Log-Secret"),
    db: AsyncSession = Depends(get_db),
):
    if AI_INVOCATION_LOG_SECRET and normalize_text(x_ai_log_secret) != AI_INVOCATION_LOG_SECRET:
        raise HTTPException(status_code=403, detail="invalid log secret")
    await _safe_append_ai_invocation_log(db, payload if isinstance(payload, dict) else {})
    return {"ok": True}
...
async def ai_web_search(payload: main.AIWebSearchRequest, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    actor = main._require_ai_session(request)
    config = await _load_ai_shared_config(db)
    main._refresh_ai_shared_config_cache(config)
    try:
        result = main._run_web_search(payload.query, payload.limit)
    except HTTPException as exc:
        await _safe_append_ai_invocation_log(
            db,
            {
                "source": "platform",
                "endpoint": "/api/ai/web-search",
                "username": actor,
                "role": await resolve_user_role(db, actor) or "",
                "success": False,
                "status_code": exc.status_code,
                "latency_ms": (time.perf_counter() - started) * 1000.0,
                "prompt_chars": len(str(payload.query or "")),
    ...
    await _safe_append_ai_invocation_log(
        db,
        {
            ...
            "prompt_chars": len(str(payload.query or "")),
            "used_search": True,
            "search_provider": str(result.get("provider") or ""),
            "search_depth": str(result.get("search_depth") or ""),
            "search_cached": bool(result.get("cached")),
            "search_result_count": len(result.get("results") or []) if isinstance(result.get("results"), list) else 0,
            "metadata_json": {
                **_request_metadata(request),
                "resolved_query": str(result.get("resolved_query") or "")[:500],
            },
        },
    )
    return result
...
async def ai_chat_with_search(
    ...
    if not await _is_known_user(db, username):
        raise HTTPException(status_code=404, detail="用户不存在")
    role = await resolve_user_role(db, username) or ""
    ...
    try:
        ...
        await _safe_append_ai_invocation_log(
            ...
            {
                "source": "platform",
                "endpoint": "/api/ai/chat-with-search",
                "username": username,
                "role": role,
                "model": model,
                "provider": base_url,
                ...
                "used_search": bool(need_web_search),
                ...
                "metadata_json": {
                    **_request_metadata(request),
                    "search_decision_reason": str(search_decision_reason)[:500],
                    "search_resolved_query": str(search_resolved_query or "")[:500],
                },
            },
        )
        raise
...
router.add_api_route("/api/internal/ai-invocation-logs", ingest_ai_invocation_log, methods=["POST"])
