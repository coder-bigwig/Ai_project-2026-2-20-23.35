...
AI_CHAT_HISTORY_LEGACY_KEY = "ai_chat_history"
AI_CHAT_HISTORY_KEY_PREFIX = f"{AI_CHAT_HISTORY_LEGACY_KEY}:"
...
def _ai_chat_history_key(username: str) -> str:
    return f"{AI_CHAT_HISTORY_KEY_PREFIX}{username}"


def _normalize_history_payload(payload: dict | None) -> list[dict[str, str]]:
    if not isinstance(payload, dict):
        return []
    raw_items = payload.get("messages")
    if raw_items is None:
        raw_items = payload.get("items")
    return main._normalize_chat_history_items(raw_items)


async def _load_ai_chat_history(db: AsyncSession, username: str) -> list[dict[str, str]]:
    repo = KVStoreRepository(db)
    row = await repo.get(_ai_chat_history_key(username))
    if row is not None:
        return _normalize_history_payload(row.value_json if isinstance(row.value_json, dict) else {})

    legacy_row = await repo.get(AI_CHAT_HISTORY_LEGACY_KEY)
    legacy_payload = legacy_row.value_json if legacy_row and isinstance(legacy_row.value_json, dict) else {}
    return main._normalize_chat_history_items(legacy_payload.get(username))


async def _save_ai_chat_history(db: AsyncSession, username: str, messages: list[dict[str, str]]) -> None:
    payload = {
        "version": 1,
        "username": username,
        "messages": messages,
    ...
    await KVStoreRepository(db).upsert(_ai_chat_history_key(username), payload)
    await db.commit()
...
async def get_ai_chat_history(username: str, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    messages = await _load_ai_chat_history(db, normalized_user)
...
async def update_ai_chat_history(
    ...
    await _save_ai_chat_history(db, normalized_user, normalized_messages)
    return main.AIChatHistoryResponse(
        username=normalized_user,
