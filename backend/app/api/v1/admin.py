...
async def list_admin_ai_invocation_logs(admin_username: str, limit: int = 200, db: AsyncSession = Depends(get_db)):
    return await _service(db).list_admin_ai_invocation_logs(admin_username=admin_username, limit=limit)
...
async def list_resource_folders(
    ...
    return await _service(db).list_resource_folders(
        teacher_username=teacher_username,
...
async def create_resource_folder(
    payload: main.ResourceFolderCreateRequest,
    db: AsyncSession = Depends(get_db),
):
    return await _service(db).create_resource_folder(payload=payload)


async def update_resource_folder(
    folder_id: str,
    payload: main.ResourceFolderUpdateRequest,
    db: AsyncSession = Depends(get_db),
):
    return await _service(db).update_resource_folder(folder_id=folder_id, payload=payload)


async def delete_resource_folder(folder_id: str, teacher_username: str, db: AsyncSession = Depends(get_db)):
    return await _service(db).delete_resource_folder(folder_id=folder_id, teacher_username=teacher_username)
...
router.add_api_route("/api/admin/ai-invocation-logs", list_admin_ai_invocation_logs, methods=["GET"])
router.add_api_route("/api/admin/operation-logs", cleanup_admin_operation_logs, methods=["DELETE"])
router.add_api_route("/api/admin/resource-folders", list_resource_folders, methods=["GET"])
router.add_api_route("/api/admin/resource-folders", create_resource_folder, methods=["POST"])
router.add_api_route("/api/admin/resource-folders/{folder_id}", update_resource_folder, methods=["PATCH"])
router.add_api_route("/api/admin/resource-folders/{folder_id}", delete_resource_folder, methods=["DELETE"])
