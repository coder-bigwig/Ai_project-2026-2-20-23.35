...
- JupyterHub 数据卷
- 上传/课程资源卷
- DeepTutor 多用户数据卷 `deeptutor-multi-user`：包含 `users.json`、每个用户目录以及聊天记录 `chat_history.db`

> 卷名以你的 compose 文件为准，可用 `docker volume ls` 查看。

### 6.2 备份 Postgres 和 DeepTutor 聊天记录（建议定期做）
Postgres 备份只包含平台数据库；DeepTutor 聊天记录不在 PostgreSQL 里，而是在 Docker volume `deeptutor-multi-user` 的 `/app/multi-user/<uid>/chat_history.db`。
...
# 1. 短暂停止会写入 PostgreSQL 或 DeepTutor 多用户目录的服务，避免备份不一致
docker compose -f docker-compose.server.yml stop deeptutor experiment-manager jupyterhub

# 2. 备份 PostgreSQL
docker compose -f docker-compose.server.yml exec -T postgres \
  pg_dump -U jupyterhub -d jupyterhub > "backups/postgres-${TS}.sql"

# 3. 备份 DeepTutor 多用户 volume，包含每个用户的 chat_history.db
docker compose -f docker-compose.server.yml run --rm --no-deps \
  -v "$(pwd)/backups:/backup" \
  --entrypoint sh deeptutor \
  -c "tar -C /app/multi-user -czf /backup/deeptutor-multi-user-${TS}.tar.gz ."

# 4. 恢复服务
docker compose -f docker-compose.server.yml start jupyterhub experiment-manager deeptutor
```
...
cat backups/postgres-YYYYMMDD-HHMMSS.sql | docker compose -f docker-compose.server.yml exec -T postgres \
  psql -U jupyterhub -d jupyterhub

docker compose -f docker-compose.server.yml run --rm --no-deps \
  -v "$(pwd)/backups:/backup" \
  --entrypoint sh deeptutor \
  -c "rm -rf /app/multi-user/* && tar -C /app/multi-user -xzf /backup/deeptutor-multi-user-YYYYMMDD-HHMMSS.tar.gz"
