...
function clearDeepTutorCookieBestEffort({ username, preferBeacon = false }) {
    const normalizedUsername = String(username || '').trim();
    const url = new URL(`${API_BASE_URL || ''}/api/deeptutor/logout`, window.location.origin);
...
function App() {
    ...
    const handleLogout = () => {
        ...
        clearDeepTutorCookieBestEffort({
            username: sessionUsername,
            preferBeacon: false
        });
    ...
    useEffect(() => {
        ...
        const sendStopSignal = (reason) => {
            ...
            // 注意：不能在这里清除 dt_token cookie。
            // DeepTutorFrame 先通过 /api/deeptutor/sso 设置了 cookie，
            // 然后 window.location.replace 跳转到 /chat 触发 beforeunload，
            // 如果在这里清除 cookie 会导致跳转后 deeptutor API 全部 401。
        };
