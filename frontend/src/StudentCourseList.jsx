...
const TEXT = {
    ...
    resourceSearch: '\u641c\u7d22',
    resourceUpload: '\u4e0a\u4f20\u8d44\u6e90\u6587\u4ef6',
    resourceUploading: '\u4e0a\u4f20\u4e2d...',
    resourceUploadSuccess: '\u8d44\u6e90\u6587\u4ef6\u4e0a\u4f20\u6210\u529f',
    resourceUploadError: '\u8d44\u6e90\u6587\u4ef6\u4e0a\u4f20\u5931\u8d25',
    resourceDelete: '\u5220\u9664',
    resourceDeleteConfirm: '\u786e\u8ba4\u5220\u9664\u8be5\u8d44\u6e90\u6587\u4ef6\u5417\uff1f',
    resourceDeleteSuccess: '\u8d44\u6e90\u6587\u4ef6\u5df2\u5220\u9664',
    resourceDeleteError: '\u5220\u9664\u8d44\u6e90\u6587\u4ef6\u5931\u8d25',
...
function StudentCourseList({ username, onLogout }) {
    ...
    const openDeepTutor = async () => {
        const targetUrl = '/deeptutor/';
        const tutorWindow = window.open('about:blank', '_blank');
        if (tutorWindow) {
            try {
                tutorWindow.opener = null;
            } catch (error) {
                // Ignore browser restrictions on clearing opener.
            }
        }
        ...
        try {
            ...
            console.warn('DeepTutor SSO failed, opening DeepTutor anyway', error);
        }

        if (tutorWindow && !tutorWindow.closed) {
            tutorWindow.location.replace(targetUrl);
            return;
        }
        ...
        <div className="lab-page-shell">
            <header className="lab-topbar">
                ...
                <div className="lab-user-block">
                    ...
                    <button type="button" className="lab-deeptutor-btn" onClick={openDeepTutor}>
                        {TEXT.deepTutor}
                    </button>
...
function CourseResourcePanel({ username, course, onBack }) {
    ...
    const loadFolders = useCallback(async () => {
        ...
        try {
            const response = await axios.get(`${API_BASE_URL}/api/student/courses/${courseId}/resource-folders`, {
            ...
            console.error('Failed to load course resource folders:', error);
            setFolders([]);
        } finally {
    ...
    }, [courseId, selectedFolderId, username]);
    ...
    return (
        <>
            <div className="lab-course-header">
                ...
                {selectedFolder ? (
                    <button type="button" className="lab-back-btn" onClick={backToRoot}>
                        返回课程资料根目录
                    </button>
                ) : null}
                {selectedFolder ? <span className="lab-course-summary">{selectedFolder.name}</span> : null}
            </div>
            ...
            <div className="lab-resource-panel">
                <div className="lab-resource-toolbar">
                    ...
                    <span className="lab-resource-total">
                        {selectedFolder
                            ? `文件 ${totalCount} 个`
                            ...
                            <tr>
                                ...
                                explorerRows.map((row) => (
                                    row.type === 'folder' ? (
                                        <tr key={row.key}>
                                            <td>
                                                <button type="button" className="lab-resource-folder-btn" onClick={() => openFolder(row.folder)}>
                                                    <span className="lab-resource-folder-icon" aria-hidden="true" />
                                                    <span>{row.folder.name}</span>
                                                </button>
                                            </td>
                                            <td>文件夹</td>
                                            <td>{formatDateTime(row.folder.created_at)}</td>
                                            <td>
                                                <button
                                                    type="button"
                                                    className="lab-resource-link detail"
                                                    onClick={() => openFolder(row.folder)}
                                                >
                                                    打开
                                                </button>
                                            </td>
                                        ...
                                        <tr key={row.resource.id}>
                                            <td>{row.resource.filename}</td>
                                            <td>{row.resource.file_type || '-'}</td>
                                            <td>{formatDateTime(row.resource.created_at)}</td>
                                            <td>
                                                <button
                                                    type="button"
                                                    className="lab-resource-link detail"
                                                    onClick={() => openResourceDetail(row.resource.id)}
                                                >
                                                    {TEXT.detail}
                                                </button>
                                                <button
                                                    ...
                                                    onClick={() => openDownload(row.resource)}
                                                >
                                                    {TEXT.download}
                                                </button>
                                            </td>
...
function StudentResourcePanel({ username }) {
    ...
    const handleUploadChange = async (event) => {
        ...
        setUploading(true);
        try {
            await axios.post(`${API_BASE_URL}/api/student/resources/upload`, formData, {
                params: { student_id: username },
                headers: { 'Content-Type': 'multipart/form-data' }
            });
            setSearchName('');
            setSearchType('');
            await loadResources({ name: '', fileType: '' });
            alert(TEXT.resourceUploadSuccess);
        } catch (error) {
            console.error('Failed to upload student resource:', error);
            alert(error.response?.data?.detail || TEXT.resourceUploadError);
        } finally {
            setUploading(false);
        }
    };
    ...
    const deleteResource = async (resource) => {
        if (!resource?.id || !window.confirm(TEXT.resourceDeleteConfirm)) {
            return;
        }
        try {
            await axios.delete(`${API_BASE_URL}/api/student/resources/${resource.id}`, {
                params: { student_id: username }
            });
            if (detailData?.id === resource.id) {
                setDetailVisible(false);
                setDetailData(null);
            }
            await loadResources();
            alert(TEXT.resourceDeleteSuccess);
        } catch (error) {
            console.error('Failed to delete student resource:', error);
            alert(error.response?.data?.detail || TEXT.resourceDeleteError);
        }
    };
        ...
        <div className="lab-resource-panel">
            <div className="lab-resource-toolbar">
                ...
                <div className="lab-resource-actions">
                    <button type="button" className="lab-resource-upload-btn" onClick={openUpload} disabled={uploading}>
                        {uploading ? TEXT.resourceUploading : TEXT.resourceUpload}
                    </button>
                    <input
                        ref={fileInputRef}
                        type="file"
                        className="lab-resource-file-input"
                            ...
                            resources.map((resource) => (
                                <tr key={resource.id}>
                                    ...
                                    <td>
                                        ...
                                        <button
                                            type="button"
                                            className="lab-resource-link delete"
